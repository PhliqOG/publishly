...
export type PostWorkflowV108Input = {
  taskQueue: string;
  postId: string;
...
function serializableActivityFailure(error: unknown) {
  const cause =
    error instanceof ActivityFailure && error.cause instanceof ApplicationFailure
      ? error.cause
      : undefined;
...
export async function postWorkflowCoreV108({
  taskQueue,
  postId,
  organizationId,
  postNow = false,
}: PostWorkflowV108Input) {
  ...
  const { postSocialPending, finalizePost, postComment } =
  ...
  for (let i = 0; i < postsList.length; i++) {
    ...
    for (let attempt = 0; attempt < iterate.length; attempt++) {
      ...
      try {
        ...
        if (
          handle.type === 'retry' &&
          attempt < iterate.length - 1
        ) {
          await waitForRecoverableRetry(err, attempt);
          continue;
        }

        // Queue/gate activities run before the provider mutation. Their
        // timeout or infrastructure failure is safe to retry because no post
        // bytes were sent.
        if (
          !mutationInvoked &&
          (handle.type === 'timeout' || handle.type === 'unknown') &&
          attempt < iterate.length - 1
        ) {
...
/**
 * V108 owns all new publishing histories. Its core is a child so the outer
 ...
 */
export async function postWorkflowV108(input: PostWorkflowV108Input) {
  const child = await startChild(postWorkflowCoreV108, {
    workflowId: `post_v108_core_${input.postId}_${workflowInfo().runId}`,
    args: [input],
  });
  ...
  let result: Awaited<ReturnType<typeof postWorkflowCoreV108>> | undefined;
  let workflowError:
    | ReturnType<typeof serializableActivityFailure>
    | undefined;
  try {
