...
import { accountQueueFailureDisposition } from '@gitroom/nestjs-libraries/reliability/account.queue.policy';
...
const {
  ...
  waitForPublishingRateLimitV108,
  schedulePublishingRetryV108,
  acquireAccountPublishingQueueV109,
  releaseAccountPublishingQueueV109,
  ensureClassifiedPublishingOutcomeV107,
  assertBulkCampaignDispatchGateV109,
...
export type PostWorkflowV109Input = {
  taskQueue: string;
  postId: string;
...
function serializableActivityFailure(error: unknown) {
  const cause =
  ...
  if (!error || typeof error !== 'object') {
    return {
      message: String(error || 'The publishing activity stopped unexpectedly.'),
  ...
  return {
    message:
      ...
      'The publishing activity stopped unexpectedly.',
    type:
      cause?.type ||
      (typeof candidate.name === 'string' ? candidate.name : 'ActivityFailure'),
    ...(cause?.details ? { details: cause.details } : {}),
  };
}

export async function postWorkflowCoreV109({
  taskQueue,
  postId,
  organizationId,
  postNow = false,
}: PostWorkflowV109Input) {
  ...
  const { checkPostStatus, reconcileAmbiguousPostV109 } =
    proxyCheckTaskQueue(taskQueue);

  const { postSocialPending, postSocialPendingV109, finalizePost, postComment } =
    proxyMutationTaskQueue(taskQueue);
  ...
  const waitForRecoverableRetry = async (
    error: unknown,
    retryOrdinal: number,
    safeBeforeMutation = false
  ) => {
    const retry = await schedulePublishingRetryV108(
      organizationId,
      postsList[0].id,
      serializableActivityFailure(error),
      retryOrdinal,
      safeBeforeMutation
    );
    await sleep(`${retry.delaySeconds} seconds`);
  };

  const waitForConnectionRateLimit = async () => {
    while (true) {
      const gate = await waitForPublishingRateLimitV108(
        organizationId,
        postsList[0].id
      );
      if (!gate) return;
      await sleep(`${gate.delaySeconds} seconds`);
    }
  };

  const waitForBulkCampaignGate = async () => {
    if (!patched('v109-bulk-campaign-dispatch-gate-v1')) return;
    while (true) {
      const gate = await assertBulkCampaignDispatchGateV109(
        organizationId,
        postsList[0].id
      );
      if (gate.ready) return;
      await sleep(
        `${Math.max(1, Math.min(gate.delaySeconds || 60, 1800))} seconds`
      );
    }
  };

  const acquireAccountQueue = async () => {
    while (true) {
      const gate = await acquireAccountPublishingQueueV109(
        organizationId,
        postsList[0].id
      );
      if (gate.acquired) return gate.leaseToken;
      await sleep(
        `${Math.max(1, Math.min(gate.delaySeconds || 5, 1800))} seconds`
      );
    }
  };
  ...
  for (let i = 0; i < postsList.length; i++) {
    ...
    for (let attempt = 0; attempt < iterate.length; attempt++) {
      ...
      let accountQueueLeaseToken: string | undefined;
      const releaseAccountQueue = async (
        outcome: 'RETRY' | 'COMPLETED' | 'FAILED' | 'AMBIGUOUS',
        code: string,
        reason: string
      ) => {
        if (!accountQueueLeaseToken) return;
        await releaseAccountPublishingQueueV109(
          organizationId,
          postsList[0].id,
          accountQueueLeaseToken,
          outcome,
          code,
          reason
        );
        accountQueueLeaseToken = undefined;
      };
      try {
        await waitForBulkCampaignGate();
        await waitForConnectionRateLimit();
        ...
        if (i === 0) {
          ...
          postsResults.push(
            ...(patched('v109-durable-attempt-readback-v1')
              ? await postSocialPendingV109(
                  post.integration as Integration,
                  [postsList[i]],
                  {
                    attemptNumber: attempt + 1,
                    activityKey: `v109:mutation:${postsList[i].id}:${attempt + 1}`,
                  }
                )
        ...
        const handle = await handleActivityError(err);
        const disposition = accountQueueFailureDisposition({
          type: handle.type,
          message: handle.message,
          mutationInvoked,
          retriesRemain: attempt < iterate.length - 1,
        });

        if (
          disposition.markUnconfirmed &&
          i === 0 &&
          patched('v109-durable-attempt-readback-v1')
        ) {
          let readback:
            | Awaited<ReturnType<typeof reconcileAmbiguousPostV109>>
            | undefined;
          try {
            readback = await reconcileAmbiguousPostV109(
              post.integration as Integration,
              postsList[0].id,
              attempt + 1
            );
            ...
            readback = {
              status: 'inconclusive',
              method: 'reconciliation_activity_failed',
              reason:
                readbackError instanceof Error
                  ? readbackError.message
                  : 'The provider readback activity failed without a verdict.',
            };
          }
          ...
          if (readback.status === 'confirmed') {
            await releaseAccountQueue(
              'COMPLETED',
              'provider_acceptance_reconciled',
              'Provider readback confirmed the timed-out mutation was accepted.'
            );
            const reconciled = {
              id: postsList[0].id,
              postId: readback.providerPostId,
              releaseURL: readback.providerUrl,
              status: 'success',
            };
            postsResults.push(reconciled);
            posted = true;
            await updatePost(
              postsList[0].id,
              reconciled.postId,
              reconciled.releaseURL
            );
            updated = true;
            await inAppNotification(
              post.integration.organizationId,
              ...
              `Provider readback confirmed your post at ${reconciled.releaseURL}`,
              true,
              true
            );
            break;
          ...
          if (readback.status === 'absent') {
            await releaseAccountQueue(
              'RETRY',
              'provider_absence_proved',
              readback.reason
            );
            await waitForRecoverableRetry(err, attempt, true);
            continue;
          }

          await releaseAccountQueue(
            'AMBIGUOUS',
            'provider_readback_inconclusive',
            readback.reason
          );
          try {
            await markUnconfirmed(readback.reason);
          } catch (e) {
            /**empty**/
          }
          return false;
        ...
        if (disposition.shouldRetry) {
          await waitForRecoverableRetry(
            err,
            attempt,
            disposition.safeBeforeMutation
          );
          continue;
        }

        // A mutation timeout or unknown error can mean the post is already
        // live. The queue enters cooldown and the publish is never replayed.
        if (disposition.markUnconfirmed) {
          try {
            await markUnconfirmed(err);
          } catch (e) {
            /**empty**/
...
/**
 * V109 owns all new publishing histories. Its core is a child so the outer
 * workflow can always run the classified terminal invariant, even when an
 * unexpected activity/workflow exception escapes the delivery flow.
 */
export async function postWorkflowV109(input: PostWorkflowV109Input) {
  const child = await startChild(postWorkflowCoreV109, {
    workflowId: `post_v109_core_${input.postId}_${workflowInfo().runId}`,
    args: [input],
  });
  ...
  let result: Awaited<ReturnType<typeof postWorkflowCoreV109>> | undefined;
  let workflowError: ReturnType<typeof serializableActivityFailure> | undefined;
  try {
    result = await child.result();
  } catch (error) {
    workflowError = serializableActivityFailure(error);
  }
