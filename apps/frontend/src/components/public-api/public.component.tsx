...
const getMcpConfig = (
  ...
  if (method === 'path') {
    switch (client) {
      case 'Claude Code':
        return {
          config: `claude mcp add publishly --transport http "${urlWithKey}"`,
          hint: 'Run this command in your terminal.',
        };
      case 'Cursor':
        return {
          config: json({ mcpServers: { publishly: { url: urlWithKey } } }),
          hint: 'Add to .cursor/mcp.json in your project root.',
        };
      case 'VS Code / Copilot':
        return {
          config: json({
            servers: { publishly: { type: 'http', url: urlWithKey } },
          }),
      ...
      case 'Windsurf':
        return {
          config: json({
            mcpServers: { publishly: { serverUrl: urlWithKey } },
          }),
          hint: 'Add to ~/.codeium/windsurf/mcp_config.json',
        };
      case 'Amp':
        return {
          config: `amp mcp add publishly ${urlWithKey}`,
          hint: 'Run this command in your terminal.',
        };
      case 'Codex':
        return {
          config: `# ~/.codex/config.toml\n\n[mcp_servers.publishly]\nurl = "${urlWithKey}"`,
      ...
      case 'Warp':
        return {
          config: json({ publishly: { url: urlWithKey } }),
  ...
  switch (client) {
    case 'Claude Code':
      return {
        config: `claude mcp add --transport http publishly ${urlBase} --header "Authorization: ${bearer}"`,
        hint: 'Run this command in your terminal.',
      };
    case 'Cursor':
      return {
        config: json({
          mcpServers: {
            publishly: { url: urlBase, headers: { Authorization: bearer } },
          },
        }),
        hint: 'Add to .cursor/mcp.json in your project root.',
      };
        ...
        <div>
          ...
          <div className="text-[13px] text-customColor18 mt-[2px]">
            {t(
              ...
              'Connect the Publishly MCP server to your client (HTTP streaming) to publish, schedule, and inspect delivery health.'
            )}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col gap-[40px]">
      <div className="text-[14px] text-textColor leading-[1.7]">
        ...
        {t(
          'api_auth_note_line2',
          'If you are building a product that schedules posts on behalf of other Publishly users,'
        )}
        <br />
        {t(
          'api_auth_note_line3',
          ...
          <div>
            ...
            <div className="text-[13px] text-customColor18 mt-[2px]">
              {t(
                ...
                'Use the Publishly API to integrate with your tools.'
              )}
            </div>
          </div>
          <div className="flex gap-[6px] shrink-0 pt-[2px]">
