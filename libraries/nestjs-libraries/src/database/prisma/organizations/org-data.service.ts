...
export class OrgDataService {
  ...
  async exportData(organizationId: string) {
    const CAP = 5000;
    const [organization, members, integrations, posts, media, webhooks, sets, signatures, auditLogs] =
      await Promise.all([
        ...
        this._prisma.media.findMany({
          where: { organizationId },
          take: CAP,
          select: { id: true, name: true, path: true, type: true, createdAt: true },
        }),
    ...
    return {
      ...
      truncated: postCount > CAP ? { posts: { total: postCount, included: CAP } } : null,
      organization,
      members,
      integrations,
      posts,
  ...
  async requestDeletion(organizationId: string) {
    ...
    await this._prisma.$transaction([
      ...
      this._prisma.integration.updateMany({
        where: { organizationId },
        data: {
          token: '',
          refreshToken: '',
      ...
      this._prisma.apiKey.updateMany({
        where: { organizationId, revokedAt: null },
        data: { revokedAt: now },
      }),
      this._prisma.organization.updateMany({
        where: { id: organizationId },
      ...
      this._prisma.webhooks.updateMany({
        where: { organizationId },
        data: { deletedAt: now },
      }),
      ...
      this._prisma.userOrganization.updateMany({
        where: { organizationId },
        data: { disabled: true },
      }),
    ]);
