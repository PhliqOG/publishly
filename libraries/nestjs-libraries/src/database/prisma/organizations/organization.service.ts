...
export class OrganizationService {
  ...
  async addTeamMemberByEmail(org: Organization, body: AdminAddTeamMemberDto) {
    ...
    if (!users.length) {
      throw new HttpException('No Publishly account found for this email', 400);
    }
  ...
  async deleteTeamMember(org: Organization, userId: string) {
    ...
    if (userRole === 'SUPERADMIN') {
      throw new HttpException(
        'The workspace owner cannot be removed. Transfer ownership before removing this account.',
        400
      );
    }
  ...
  transferOwnership(
    ...
    return this._organizationRepository.transferOwnership(
      orgId,
      currentOwnerId,
      targetUserId
    );
